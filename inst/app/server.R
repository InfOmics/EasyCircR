...
optionstable = list(lengthChange = FALSE, paging = FALSE, info=FALSE)

#library(clusterProfiler)


colnames_all_tbl = c("circRNA", "miRNA", "Gene","Gene logFC", "Gene PValue",
                     "validation miRNA-Gene", "database miRNA-Gene", 
                     "miRNA family", "circRNA-miRNA binding start", 
                     "circRNA-miRNA binding end", "type of miRNA target site",
                     "Gene set")
...
generate_circos_plot <- function(circ, mirna, dir){
  
  circ$circ_start <- 0
  circ$circ_end <- circ$circ_start + circ$length
  list_circ <- circ$circRNA_id
  
  for(c in list_circ){
    # creation mirna binding dataframe
    tp <- mirna[which(mirna$circRNA_id == c),c("circRNA_id", "Binding_start", "Binding_end", "mature_mirna_id")]
    tp <- tp[!duplicated(tp),]
    colnames(tp)[1] <- "sector"

    # take just one circ
    circular_object <-  circ[which(circ$circRNA_id == c), c("circRNA_id", "circ_start", "circ_end")]
    circular_object <- circular_object[1,]
    circular_object$circ_start <- as.numeric(circular_object$circ_start)
    circular_object$circ_end <- as.numeric(circular_object$circ_end)

    png(paste(paste(dir, c, sep = "/"), ".png", sep = ""), res = 300, width = 3000, height = 3000)
    # cicular structure creation
    
    circlize::circos.genomicInitialize(circular_object, axis.labels.cex = 1, labels.cex = 1)
    ...
    n = nrow(tp)
    # overlapping miRNA
    circlize::circos.genomicTrack(tp, ylim = c(0.8, n + 0.8), 
                                  panel.fun = function(region, value, ...) {
                                    for(i in 1:nrow(value)) {
                                      name = value$miRNA_family_ID[i]
                                      current_tx_start = region[i, 1]
                                      current_tx_end = region[i, 2]
                                      circlize::circos.lines(c(current_tx_start, current_tx_end), 
                                                             c(n - i , n - i), col = "#000000", type = "o", 
                                                             cex = 0.3)
                                    }
...
gene_mirna_circ <- gene_mirna_circ[,c("circRNA_link", "mature_mirna_id",
                                      "target_symbol", "logFC","PValue",
                                       "type", "database", "mirna_family",
                                      "Binding_start", "Binding_end",
                                      "Site_type", "category_H", "CIRCRNA_ID",
                                      "circRNA_id", "chr", "circ_pos_start", "circ_pos_end")]
...
# CIRCRNA
#--------------------------------------------------------------------------------
circrnas = circRNA_seq[,c("circRNA_id", "length", "validated_name")]
...
circrnas$validated_name[is.na(circrnas$validated_name)] = "-"
...
# Creation circos plot directory 
if(!file.exists("data/plot_circos/")){
  dir.create(file.path("data/plot_circos/"))
} 
  
# Create circ Plots
generate_circos_plot(circrnas, circrnas_mirna, dir="data/plot_circos/")
    ...
    gene_filt = reactive({
        if (length(rv$filter)) {
            ...
            if (length(filt_mirna)) 
                filt_gene_mirna_gene = filt_gene_mirna_gene %>% filter(mature_mirna_id %in% filt_mirna)
            if (length(filt_circ)) 
                filt_gene_mirna_gene = filt_gene_mirna_gene %>% filter(CIRCRNA_ID %in% filt_circ)
    ...
    mirna_filt = reactive({
        if (length(rv$filter)) {
            ...
            if (length(filt_circ)) 
                filt_gene_mirna_gene = filt_gene_mirna_gene %>% filter(CIRCRNA_ID %in% filt_circ)
            if (length(filt_gene)) 
                filt_gene_mirna_gene = filt_gene_mirna_gene %>% filter(target_symbol %in% filt_gene)
...
#                                   filter(support_type %in% input$flt_support_type)
        ...
        if (length(input$flt_chr)  && !is.na(as.numeric(input$flt_chr)))
            filt_gene_mirna_gene = filt_gene_mirna_gene %>%
                                   filter(as.character(input$flt_chr) == as.character(chr))

        if (length(input$flt_start_pos) && !is.na(as.numeric(input$flt_start_pos)))
            filt_gene_mirna_gene = filt_gene_mirna_gene %>%
                                   filter(as.numeric(input$flt_start_pos) == as.numeric(circ_pos_start))

        if (length(input$flt_ending_pos) && !is.na(as.numeric(input$flt_ending_pos)))
            filt_gene_mirna_gene = filt_gene_mirna_gene %>%
                                   filter(as.numeric(input$flt_ending_pos) == as.numeric(circ_pos_end))
    ...
    all_tbl <- reactive({
        ...
        datatable(datab[, -which(colnames(datab) == "circRNA_id" | colnames(datab) == "chr" |
                                   colnames(datab) == "circ_pos_start" | colnames(datab) == "circ_pos_end")],
...
#            datatable(rv$genes %>% arrange(PValue),
    ...
    output$circ_tbl <- DT::renderDataTable({
      circrnas <-  circrnas %>% mutate(
        structure = shinyInput(actionButton, nrow(circrnas), 'button_', label = "View",
                               onclick = 'Shiny.onInputChange(\"select_button\",  this.id)' )
      
      )
      datatable(circrnas,
                colnames=c("circRNA", "circRNA length (bp)", "validated name","number of\nmiRNAs binding", "structure"),
                escape = FALSE,
                options = optionstable,
                rownames= FALSE,
                selection = list(mode = 'single', target = 'row')
...
#------------------------------------------------------------------------------
    ...
    # pop-up circRNA plots
    observeEvent(input$select_button, {
      selectedRow <- as.numeric(strsplit(input$select_button, "_")[[1]][2])
      rv$circ_to_plot <- circrnas$circRNA[selectedRow]
      showModal(modalDialog( title = circrnas$circRNA[selectedRow],
                             footer = modalButton("Close"),
                             box(title = "miRNA binding sites", solidHeader=T,status =
                                          "primary", width="12",
                                                column(width=12,
                                                       renderImage({
                                                         circ_id = rv$circ_to_plot
                                                         if (!identical(circ_id, character(0))){
                                                           filename <- paste('data/plot_circos/', paste(circ_id, '.png', sep=''), sep = "")
                                                         }
                                                         list(src = filename,
                                                              width = "90%",
                                                              height = "100%")
                             ...
                             box(title = "CIRI tool reconstruction circRNA ", solidHeader=T,status =
                                   "primary", width="12", height = "600",
                                 column(width=12,
                                        renderImage({
                                          circ_id = rv$circ_to_plot
      ...
      #subsetModal(session, data = subsetted, size = "m")
    })
    ...
    output$flt_type <- renderUI({
		selectizeInput("flt_type", label="miRNA-Gene interaction", choices=levels(factor(gene_mirna_circ$type)),  multiple=TRUE,
					   options=list(plugins=list("remove_button"), create=TRUE, persist=FALSE))
	})
    # database (miRNA)
    output$flt_database <- renderUI({
		selectizeInput("flt_database", label="Database of miRNA-Gene interaction", choices=levels(factor(gene_mirna_circ$database)),  multiple=TRUE,
					   options=list(plugins=list("remove_button"), create=TRUE, persist=FALSE))
	})
    # support_type 
    ...
    output$flt_site_type <- renderUI({
		selectizeInput("flt_site_type", label="Type of miRNA target\n site", choices=levels(factor(gene_mirna_circ$Site_type)),  multiple=TRUE,
					   options=list(plugins=list("remove_button"), create=TRUE, persist=FALSE))
	})
    # category_H
    ...
    output$flt_logFC_max <- renderUI({
        textInput("flt_logFC_max", label="Gene logFC cutoff")
	})
    ...
    output$flt_pval_max <- renderUI({
        textInput("flt_pval_max", label="Gene PValue cutoff")
	})

    output$flt_chr <- renderUI({
      textInput("flt_chr", label="Chromosome")
  })

    output$flt_start_pos <- renderUI({
      textInput("flt_start_pos", label="circRNA starting position")
  })
    
    output$flt_ending_pos <- renderUI({
      textInput("flt_ending_pos", label="circRNA ending position")
  })
   ...
   # })
    ...
    output$step_first <- renderValueBox({
      valueBox(
        value = ("STEP 1: Lists of circRNAs, miRNAs and genes founded"),
        subtitle = "Visualization of all the circRNAs, miRNAs and genes founded",
        color= "blue"
      )
    })
    
    output$step_second <- renderValueBox({
      valueBox(
        value = ("STEP 2: filtering of circRNAs, miRNAs and genes"),
        subtitle = "Select circRNA, miRNAs and/or genes to compose circRNA-miRNA-gene interactions",
        color= "blue"
      )
    })
    
    output$step_third <- renderValueBox({
      valueBox(
        value = ("STEP 3: Visualization and download"),
        subtitle = "Visualize and download all circRNA-miRNA-gene interactions",
        color= "blue"
      )
    })
    ...
    output$error_barplot <- renderText({ 
    ...
    # output$circular_plot <- renderImage({ 
    #   circ_id = rv$circ_to_plot
    #   circ_id = circrnas$circRNA_id[input$all_circ_tbl_rows_selected]
    #   if (!identical(circ_id, character(0))){
    #     filename <- paste(data/plot_circos, paste(circ_id, '.png', sep=''), sep = "")
    ...
    observeEvent(input$flt_clear, {
      shinyjs::reset(id = "page")
    })
