#' @title Run EasyCirc Pipeline
#'
#' @description Starting from RNA-seq data, it uses CIRI-full for the reconstruction of
...
#'
#'   1. Measure changes across different experimental conditions to quantify post-transcriptional regulation of gene expression (EISA).
...
#'   4. Predict MREs from the full sequence of circRNA (TargetScan, miRanda, RNA-hybrid).
#'   5. Finally, connecting circRNA to genes.
#'
#' @author Luca Parmigiani
#'
...
#' @param eisa_n_core number of core to be used in the EISA step. Default is \code{1}. 
#' @param cirifull_n_core number of core to be used in the CIRI-Full step. Default is \code{1}.
#' @param percent_of_reads_after_trim Percentage of reads to be kept after the trimming of the #' reads. Default is \code{.8}.  
...
#' @param lfc Minimum absolute log2-fold-change required for circRNA and EISA differential
#' expressed step.
#' @param p.value cutoff value for adjusted p-values. Only genes/circRNAs with lower
#' p-values are listed.
#' @param mirna_gene_type used in the prediction between miRNA and gene interaction. 
...
#' @param predicted.cutoff 'NULL' or an integer giving a prediction
#' score cutoff.  By default the cutoff is '32'.
#' @param eisa_aligner selects the aligner program to be used for aligning the
#' reads for the eisa step. Currently, only “Rbowtie” and “Rhisat2” (default) are
#' supported.
#' @param eisa_stranded_data \code{logical(1)}. If \code{TRUE}, the RNA-seq
...
#' out.
#' @param circrna_filter_min_num_samples \code{numeric}. Before computing the
...
#' @param voomWithQualityWeights \code{logical(1)}. Use
#' \code{voomWithQualityWeights} instead of \code{voom} during circRNA DE
#' analisis.  
#' @param remove_temporary_files \code{logical(1)}. If \code{TRUE} (Default) alignment
...
#'
#' @export
run_easycirc <- function (samples_file, 
                          genome_file, genome_annotation_file,
                          condition, 
                          force=FALSE, 
                          eisa_n_core=1, cirifull_n_core=1,
    ...
    cat("Step 1: Running EISA\n")
    post_gene <- get_postregulated_gene(samples_file, 
                                        genome_file, genome_annotation_file,
                                        condition, 
                                        force=force, 
                                        n_core=eisa_n_core, 
                                        aligner=eisa_aligner,
                                        stranded_data=eisa_stranded_data,
                                        only_significant_genes=TRUE)

    cat("Step 2: Running CIRI-Full\n")
    #if (is.null(trim_reads_length)) {
    #   trim_reads_length
    #}
    run_ciri_full(samples_file, genome_file, genome_annotation_file, 
              trim_reads_length=trim_reads_length, force=force, remove_temporary_files=remove_temporary_files)

    circ <- read_ciri_output(samples_file)
    circ_df <- circ$circ_df 
    circ_mtx <- circ$circ_mtx

    cat("Step 3: DE circRNAs\n")
    design <- model.matrix(~0+condition)
    contr <- (colnames(design)==colnames(design)[2] ) - (colnames(design)==colnames(design)[1])

    circ_de <- de_circrna(circ_mtx, condition, design, contr, lfc=lfc, p.value=p.value,
                      voomWithQualityWeights=voomWithQualityWeights, 
                      min_num_samples=circrna_filter_min_num_samples)

    cat("Step 4: circRNA MREs prediction\n")
    circ_mirna <- get_mirna_binding(circ_df_de, force=force)

    cat("Step 5: circRNA-miRNA-gene\n")
    gene_mirna_circ <- connect_circ_gene(circ_mirna, post_gene, only_significant_genes=TRUE, force=force)

    return(list(circ_de=circ_de, gene_de=post_gene, circ_df=circ_df, gene_mirna_circ=gene_mirna_circ))
}
